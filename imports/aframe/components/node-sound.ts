...
AFRAME.registerComponent('node-sound', {
  schema: {
    ...
    bufferSize: { type: 'number', default: 256 },
    sampleRate: { type: 'number', default: 44100 },
    latencyHint: { type: 'string', default: 'interactive' },
    numberOfChannels: { type: 'number', default: 2 },
    synchronization: { type: 'string', default: 'auto' }
  },

  init: function () {
    console.log("node sound init")
    this.audioContext = new (window.AudioContext || window.webkitAudioContext)({
      latencyHint: this.data.latencyHint,
      sampleRate: this.data.sampleRate,
      bufferSize: this.data.bufferSize,
      numberOfChannels: this.data.numberOfChannels,
      synchronization: this.data.synchronization
    });
    const self = this;
  ...
  tick: function () {
    ...
    const { x, y, z } = this.el.object3D.getWorldPosition();
    ...
    if (this.camera && this.camera.object3D) {
      const { x: cameraX, y: cameraY, z: cameraZ } = this.camera.object3D.getWorldPosition();
